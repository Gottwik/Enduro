...
admin_file_upload_handler.prototype.upload = function (file) {
	...
	if (enduro.config.variables.S3_KEY && enduro.config.variables.S3_SECRET) {
...
admin_file_upload_handler.prototype.upload_by_url = function (file_url) {
	...
	return new Promise(function (resolve, reject) {
		...
		if (flat_helpers.is_local(file_url)) {
			...
			var destination = path.join(enduro.project_path, 't', filename)
...
function uploadfile_local (file) {
	...
	return new Promise(function (resolve, reject) {
		var destination_path = path.join(enduro.project_path, UPLOADS_FOLDER, (new Date() / 1e3 | 0) + '_' + file.name)
		var destination_src_path = path.join(enduro.project_path, '_src', UPLOADS_FOLDER, (new Date() / 1e3 | 0) + '_' + file.name)
